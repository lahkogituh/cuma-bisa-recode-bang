...
┏━━━ꕥ〔 *PEMBAYARAN* 〕ꕥ━⬣
┃✾ *Dana :* 0823-3460-2688
┃✾ *Gopay :* 0823-3460-2688
┃✾ *Ovo :* 0823-3460-2688
┃✾ *Pulsa :* 0823-3460-2688 
┗━ꕥ`.trim(), footer, 'Dana', '#viadana', 'Owner', '#owner', m)
    ...
    if (isSalam && !m.fromMe) {
        ...
        let res = await fetch(`https://github.com/buatngetes/vnnk/raw/master/walaikunsalam.mp3`)
     json = await res.buffer()
     await conn.sendFile(m.chat, json, 'error.mp3', null, m, true)
    }
    if (isTes && !m.fromMe) {
        m.reply(`Iya sayang sudah aktif kok:')`)
        let res = await fetch(`https://github.com/buatngetes/vnnk/raw/master/moshimossh.mp3`)
     json = await res.buffer()
     await conn.sendFile(m.chat, json, 'error.mp3', null, m, true)
    }
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`you.`).catch(_ => _)
